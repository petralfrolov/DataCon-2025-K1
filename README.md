...
Используем базу **ChEMBL** как основную и наиболее авторитетную, **BindingDB** как дополнительную. За счет присоединения данных из **BindingDB** удалось улучшить метрики моделей машинного обучения на **~10%**.
...
Для отбора генерируемых молекул мы разработали фильтр с различными метриками:
| Фильтр           | Порог                                                  |
| ---------------- | ------------------------------------------------------ |
...
Для начала мы попробовали модель **DeepSeek**, так как думали, что она сможет сгенерировать SMILES быстрее и в большем объеме, чем локально запускаемые модели.
...
* Юзер-промпт:
    f"Сгенерируй {n_samples} уникальных SMILES. Один SMILES = одна строка."
* 10 прогонов × 30-50 молекул → \~400 строк
* валидность SMILES ≈ 50-90 %
* SMILES однообразны и похожи друг на друга
* После фильтров осталась **1** молекула, прошедшая все критерии (см. файл hits_deepseek.xlsx)

**Плюсы**:
* Простота настройки - написал промпт и поехал

**Минусы**:
* <50 мол. за прогон - независимо от кол-ва запрашиваемых молекул, генерирует рандомное кол-во и не больше 50-ти
* 10-50% мусора - часть молекул невалидны
* Однообразные молекулы - все молекулы, даже из разных прогонов, похожи друг на друга
* Скорость работы - хоть работает через онлайн API, на одну генерацию уходит ~30 секунд


### 2. REINVENT 4

Когда стали очевидны минусы стандартных GPT-моделей, мы начали искать специально заточенные под генерацию молекул варианты. Наиболее современной из доступных моделей оказалась **REINVENT 4**, её мы и протестировали после **DeepSeek**. Мы решили не пробовать дообучить модель (во-первых качественный результат "из коробки" за 12 секунд, во-вторых - мало времени до конца хакатона).

Результаты:
* валидность SMILES ≈ 100 %
* **50 000** молекул:
* После фильтров — 51 хит (см. файл hits_reinvent4.xlsx), 3 лучших были взяты для докинга.

**Плюсы**:
* Много мол. за прогон - нет ограничения на кол-во генерируемых молекул
* 0% мусора - все сгенерированные молекулы валидны
* Разнообразные молекулы
* Скорость работы - 50000 молекул на cuda сгенерировалось за ~12 секунд
* Актуальность модели: выпущена AstraZeneca в 2024

**Минусы**:
* Сложность настройки - надо настраивать venv для корректной работы
...
* **Софты**: AutoDock‑Vina 1.2.5.
* **Лучший гибрид**
  *SMILES*: `Cc1ccc(OCC(=O)NC2CCN(Cc3ccccc3)CC2)cc1`

**AChE**
* Лучший результат – –8,24 ккал/моль.
* Остальные восемь поз отличаются по энергии не больше чем на ~1 ккал/моль (–8,03 … –7,29), что указывает на устойчивый набор сходных конфигураций.
* RMSD ≈ 2 Å у поз 2‑5 говорит о плотном главном кластере; позы 6‑9 (RMSD 4‑5 Å) отражают альтернативные ориентации в том же кармане.

**BACE1**
* Лучший результат – –8,55 ккал/моль, то есть примерно на 0,3 ккал/моль лучше, чем для AChE.
* Энергии остальных поз распределены в диапазоне –8,29 … –7,49 ккал/моль (разброс ~1 ккал/моль).
* Позы 4‑5 формируют компактный кластер (RMSD ≤ 1,7 Å); у поз 2 и 6‑9 RMSD 2‑6 Å, что указывает на второй, более разреженный набор ориентаций.

**Итог**
* Докинг прошёл без явных проблем: конвергенция есть, все найденные позы имеют умеренно благоприятную энергию (~–8 ккал/моль). Лиганд немного предпочтительнее связывается с BACE1, но разница невелика, поэтому в обоих ферментах возможна сопоставимая аффинность, с двумя основными кластерами ориентаций для каждого белка.

<img width="1269" height="727" alt="image" src="https://github.com/user-attachments/assets/9ad0f330-1f4e-4b78-9b52-7b81dca56a3f" />
